...
export default function App() {
  ...
  const [autoStrategies, setAutoStrategies] = useState(["combined"]);
  ...
  useEffect(() => {
    if (state && !autoSettingsSynced.current) {
      ...
      if (state.auto_strategies != null) setAutoStrategies(state.auto_strategies);
  ...
  const toggleAutoStrategy = (name) => {
    setAutoStrategies((prev) =>
      prev.includes(name) ? prev.filter((s) => s !== name) : [...prev, name]
    );
  };

  const toggleSymbolStrategy = (sym, name) => {
    setSymbolConfigDraft((prev) => {
      const cur = prev[sym] || {};
      const list = cur.strategies || [];
      const next = list.includes(name) ? list.filter((s) => s !== name) : [...list, name];
      return { ...prev, [sym]: { ...cur, strategies: next } };
    });
  };

  const setSymbolGranularity = (sym, gran) => {
    setSymbolConfigDraft((prev) => ({ ...prev, [sym]: { ...(prev[sym] || {}), granularity_sec: Number(gran) } }));
  };

  const saveSymbolConfig = async () => {
    try {
      const payload = {};
      for (const [sym, cfg] of Object.entries(symbolConfigDraft)) {
        const entry = {};
        if (cfg.strategies && cfg.strategies.length) entry.strategies = cfg.strategies;
        if (cfg.granularity_sec) entry.granularity_sec = cfg.granularity_sec;
  ...
  return (
    <div className="app" data-testid="app">
      ...
      <div className="risk-card">
        ...
        <div style={{ marginTop: 12, padding: "10px 12px", border: "1px solid var(--border, #2a2d36)", borderRadius: 8 }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 12 }}>STRATEGIE ATTIVE (AUTO-TRADING)</div>
          <div style={{ fontSize: 11, color: "var(--text2, #888)", marginTop: 2, marginBottom: 10 }}>
            Ognuna selezionata agisce IN MODO INDIPENDENTE: basta che una sola dia un segnale confermato per aprire un trade — non serve che siano tutte d'accordo. "ICT + PA" resta comunque una combinazione interna delle due.
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {(s.available_strategies || ["ict", "price_action", "combined", "trend_following", "mean_reversion", "breakout", "indicators"]).map((name) => {
              const active = autoStrategies.includes(name);
              return (
                <button
                  key={name}
                  data-testid={`auto-strategy-toggle-${name}`}
                  onClick={() => toggleAutoStrategy(name)}
      ...
      <div className="risk-card">
        <div style={{ fontSize: 11, color: "var(--text2, #888)", marginBottom: 10 }}>
          Ogni simbolo può avere le proprie strategie e il proprio timeframe candele,
          indipendenti da quelli globali qui sopra — es. Mean Reversion su XAU/USD a 1 ora,
          Breakout su BTC/USD a 1 ora. Un simbolo senza strategie selezionate qui sotto usa i
          default globali.
        </div>
        {(s.watchlist && s.watchlist.length ? s.watchlist : Object.keys(SYMBOL_LABELS)).map((sym) => {
          const cfg = symbolConfigDraft[sym] || {};
          const hasOverride = cfg.strategies && cfg.strategies.length > 0;
          return (
            <div key={sym} style={{ border: "1px solid var(--border, #2a2d36)", borderRadius: 8, padding: 10, marginBottom: 10 }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>{SYMBOL_LABELS[sym] || sym}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                {(s.available_strategies || ["ict", "price_action", "combined", "trend_following", "mean_reversion", "breakout", "indicators"]).map((name) => {
                  ...
                  return (
                    <button
                      key={name}
                      data-testid={`symbol-strategy-toggle-${sym}-${name}`}
                      onClick={() => toggleSymbolStrategy(sym, name)}
                      className={`env-btn ${active ? "active" : ""}`}
                      style={{ fontSize: 10 }}
                    >
                      {STRATEGY_LABELS[name] || name}
              ...
              <select
                ...
                onChange={(e) => setSymbolGranularity(sym, e.target.value)}
              ...
              <div style={{ fontSize: 10, color: "var(--text2, #888)", marginTop: 6 }}>
                {hasOverride ? "Configurazione specifica attiva per questo simbolo." : "Nessuna strategia selezionata qui: userà le strategie globali sopra."}
              </div>
            </div>
          );
        })}
      ...
      <div className="risk-card">
        <div style={{ fontSize: 11, color: "var(--text2, #888)", marginBottom: 10 }}>
          Scarica lo storico REALE di {SYMBOL_LABELS[s.active_symbol] || s.active_symbol} da Deriv e simula
          come si sarebbero comportate le strategie selezionate, con gli stessi TP/SL/leva impostati sopra.
        </div>
