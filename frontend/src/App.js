...
// Un errore dal backend può essere una stringa semplice, oppure — quando FastAPI rifiuta
// la richiesta per validazione (HTTP 422) — una LISTA di oggetti Pydantic tipo
// {type, loc, msg, input, ctx}. Se quell'oggetto finisce renderizzato direttamente in JSX
// invece che come testo, React va in crash (errore #31). Questa funzione estrae sempre e
// solo una stringa leggibile, qualunque forma abbia l'errore.
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("Errore React catturato:", error, info);
  }
  render() {
    if (this.state.error) {
      return (
        <div style={{
          minHeight: "100vh", background: "#04060f", color: "#ff4d6d",
          fontFamily: "monospace", padding: 20, display: "flex", flexDirection: "column", gap: 14,
        }}>
          <div style={{ fontSize: 16, fontWeight: 700 }}>✗ Si è verificato un errore</div>
          <div style={{ fontSize: 13, whiteSpace: "pre-wrap", color: "#ffb0bc" }}>
            {String(this.state.error && (this.state.error.stack || this.state.error.message || this.state.error))}
          </div>
          <button
            onClick={() => window.location.reload()}
            style={{ padding: "12px 16px", background: "#f5c542", color: "#000", border: "none", borderRadius: 8, fontFamily: "monospace", fontWeight: 700, fontSize: 14 }}
          >
            RICARICA APP
          </button>
        </div>
...
function AppInner() {
  ...
  const [trailingActivation, setTrailingActivation] = useState(40);
  const [trailingGiveback, setTrailingGiveback] = useState(20);
  const [earlyCutPct, setEarlyCutPct] = useState(40);
  const [earlyCutConfirm, setEarlyCutConfirm] = useState(3);
  const [minVolatilityIndex, setMinVolatilityIndex] = useState(0.6);
  ...
  useEffect(() => {
    if (state && !autoSettingsSynced.current) {
      ...
      if (state.trailing_activation_pct != null) setTrailingActivation(state.trailing_activation_pct);
      if (state.trailing_giveback_pct != null) setTrailingGiveback(state.trailing_giveback_pct);
      if (state.early_cut_pct != null) setEarlyCutPct(state.early_cut_pct);
      if (state.early_cut_confirm != null) setEarlyCutConfirm(state.early_cut_confirm);
      if (state.min_volatility_index != null) setMinVolatilityIndex(state.min_volatility_index);
  ...
  const submitSetup = async () => {
    ...
    try {
      ...
      else setSetupErr(extractError(e));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const saveAutoSettings = async () => {
    try {
      const r = await axios.post(`${API}/auto-settings`, {
        ...
        trailing_activation_pct: Number(trailingActivation) || 40,
        trailing_giveback_pct: Number(trailingGiveback) || 20,
        early_cut_pct: Number(earlyCutPct) || 40,
        early_cut_confirm: Number(earlyCutConfirm) || 3,
        min_volatility_index: Number(minVolatilityIndex) || 0.6,
      });
      setState(r.data);
  ...
  return (
    <div className="app" data-testid="app">
      ...
      <div className={`sig-card ${dir !== "WAIT" ? dir : ""}`} data-testid="signal-card">
        ...
        {s.volatility && (
          <div data-testid="volatility-badge" style={{
            display: "inline-flex", alignItems: "center", gap: 6, margin: "6px 4px 0", padding: "4px 10px",
            borderRadius: 20, fontSize: 11, fontFamily: "var(--mono)",
            background: s.volatility.level === "elevata" ? "rgba(0,212,160,.12)" : s.volatility.level === "compressa" ? "rgba(255,77,109,.12)" : "rgba(255,255,255,.06)",
            color: s.volatility.level === "elevata" ? "var(--green, #00d4a0)" : s.volatility.level === "compressa" ? "var(--red, #ff4d6d)" : "var(--text2, #888)",
          }}>
            📊 Volatilità: {s.volatility.index?.toFixed(2)}× ({s.volatility.level})
          </div>
        )}
      ...
      <div className="sec-title">VOLATILITÀ PER SIMBOLO</div>
      <div className="risk-card" style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 11, color: "var(--text2, #888)", marginBottom: 10 }}>
          Indice relativo alla volatilità recente di ciascun asset (1.0 = nella norma). "Elevata" spesso segna l'inizio di un movimento direzionale — utile soprattutto per Breakout e Trend Following. Sotto la soglia minima, l'auto-trading salta i nuovi trade su quel simbolo.
        </div>
        {Object.entries(s.markets || {}).map(([sym, mk]) => {
          const v = mk.volatility || { index: 1.0, level: "normale" };
          const color = v.level === "elevata" ? "var(--green, #00d4a0)" : v.level === "compressa" ? "var(--red, #ff4d6d)" : "var(--text2, #888)";
          return (
            <div key={sym} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 4px", borderBottom: "1px solid var(--border, #2a2d36)" }}>
              <span style={{ fontSize: 12 }}>{SYMBOL_LABELS[sym] || sym}</span>
              <span style={{ fontSize: 12, fontFamily: "var(--mono)", color }}>{v.index?.toFixed(2)}× · {v.level}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="risk-card">
        ...
        <div style={{ fontSize: 11, color: "var(--text2, #888)", margin: "16px 0 10px" }}>
          <strong style={{ color: "var(--gold)" }}>Uscita intelligente</strong> — invece di aspettare sempre il TP/SL pieno: blocca un guadagno più piccolo se il prezzo comincia a tornare indietro dopo essere stato in profitto, e taglia una perdita più piccola se il mercato continua chiaramente contro la posizione. Il TP/SL pieno sopra resta comunque come rete di sicurezza finale.
        </div>
        <div className="risk-grid">
          <div className="risk-field"><label>Trailing: attiva da (%)</label><input data-testid="trailing-activation-input" type="number" value={trailingActivation} onChange={(e) => setTrailingActivation(e.target.value)} min="1"/></div>
          <div className="risk-field"><label>Trailing: margine di ritorno (%)</label><input data-testid="trailing-giveback-input" type="number" value={trailingGiveback} onChange={(e) => setTrailingGiveback(e.target.value)} min="1"/></div>
          <div className="risk-field"><label>Taglio anticipato perdita (%)</label><input data-testid="early-cut-input" type="number" value={earlyCutPct} onChange={(e) => setEarlyCutPct(e.target.value)} min="1"/></div>
          <div className="risk-field"><label>Conferme richieste</label><input data-testid="early-cut-confirm-input" type="number" value={earlyCutConfirm} onChange={(e) => setEarlyCutConfirm(e.target.value)} min="1" max="10"/></div>
        </div>

        <div style={{ fontSize: 11, color: "var(--text2, #888)", margin: "16px 0 10px" }}>
          <strong style={{ color: "var(--gold)" }}>Filtro di volatilità</strong> — un mercato troppo "piatto" tende a oscillare senza risolversi mai. Sotto questa soglia (indice relativo, 1.0 = normale), l'auto-trading salta i nuovi trade su quel simbolo finché la volatilità non torna nella norma.
        </div>
        <div className="risk-grid">
          <div className="risk-field"><label>Volatilità minima (indice)</label><input data-testid="min-volatility-input" type="number" step="0.1" value={minVolatilityIndex} onChange={(e) => setMinVolatilityIndex(e.target.value)} min="0.1"/></div>
        </div>
      ...
      <div className={`settings-panel ${settingsOpen ? "open" : ""}`}>
        ...
        <div className="settings-body">
          <div className="settings-field"><label>Nuovo Token API Deriv</label><input type="text" value={token} onChange={(e) => setToken(e.target.value)}/></div>
