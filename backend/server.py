...
class DerivClient:
    ...
    async def _maybe_signal(self, symbol: str):
        ...
        for name in (self.auto_strategies or [self.strategy]):
...
async def _fetch_candles_paginated(sym: str, granularity: int, total_count: int) -> list:
    """Deriv limita ogni singola chiamata ticks_history — per certe combinazioni
    simbolo/granularità il tetto reale è più basso dei 5000 documentati (es. ~1000).
    Ricevere MENO candele del richiesto in una pagina NON significa che lo storico sia
    finito: significa solo che va richiesta un'altra pagina più indietro nel tempo.
    L'unico segnale affidabile di storico davvero esaurito è una risposta VUOTA.
    Continuiamo quindi a paginare finché non raggiungiamo il totale richiesto o Deriv
    smette proprio di restituire candele (con un tetto di sicurezza sui giri per evitare
    loop infiniti in caso di comportamento imprevisto dell'API)."""
    all_candles: list = []
    end_param: Any = "latest"
    max_iterations = 40  # margine ampio: anche con pagine da sole ~250 candele arriva a 10000
    for _ in range(max_iterations):
        if len(all_candles) >= total_count:
            break
        remaining = total_count - len(all_candles)
        batch_count = min(DERIV_MAX_CANDLES_PER_CALL, remaining)
        ...
        batch = hist.get("candles") or []
        if not batch:
            client.log("I", f"Backtest {sym}: storico esaurito, Deriv non ha altre candele più vecchie — totale ottenuto {len(all_candles)}")
            break
        # Deriv a volte manda pagine che si sovrappongono di una candela col bordo
        # precedente: scartiamo eventuali epoch duplicati prima di unire.
        if all_candles:
            known_epochs = {c["epoch"] for c in all_candles[: min(5, len(all_candles))]}
            batch = [c for c in batch if c["epoch"] not in known_epochs]
        if not batch:
            break
        all_candles = batch + all_candles
        oldest_epoch = batch[0]["epoch"]
        end_param = oldest_epoch - granularity
        if len(batch) < batch_count:
            client.log("I", f"Backtest {sym}: pagina da {len(batch)}/{batch_count} candele (sotto il richiesto) — continuo a paginare più indietro, totale finora {len(all_candles)}")
