    ...
    async def _maybe_signal(self, symbol: str):
        ...
        for name in (self.auto_strategies or [self.strategy]):
            ...
            if d == st["pending_dir"]:
                ...
                st["pending_dir"] = d
                st["pending_count"] = 1
            confirmed = st["pending_count"] >= self.confirm_need
...
class BacktestBody(BaseModel):
    ...
    count: int = 5000                  # numero di candele storiche da scaricare (max Deriv ~5000)
...
async def run_backtest(body: BacktestBody):
    """Scarica candele storiche REALI da Deriv per il simbolo indicato e simula la
    stessa identica logica live (stesse funzioni strategia, stesso filtro
    target_reachable, stessa formula di leva) per stimare come si sarebbero
    comportate le strategie selezionate. Richiede che il bot sia connesso (usa la
    stessa sessione WebSocket già autenticata)."""
    ...
    count = min(max(body.count, 100), 5000)
    try:
    ...
    if hist.get("error"):
        raise HTTPException(502, hist["error"].get("message", "errore ticks_history"))
    raw_candles = hist.get("candles") or []
    ...
    report = backtest_engine.run_backtest(
        candles,
        strategy_names,
        confirm_need=body.confirm_need or client.confirm_need,
