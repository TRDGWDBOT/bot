...
class DerivClient:
                ...
                async with websockets.connect(url, ping_interval=None) as ws:
                    ...
                    try:
                        ...
                        for sym in self.watchlist:
                            try:
                                cf = await self._send({"contracts_for": sym, "currency": self.currency or "USD"}, timeout=15)
    ...
    async def _maybe_signal(self, symbol: str):
        ...
        if self.auto_mode and confirmed and ok and d != "WAIT" and d != m["last_auto_dir"]:
            ...
            # sullo STESSO simbolo è ancora aperta: altrimenti BUY e SELL si annullano a
            # vicenda e il conto perde solo spread/commissioni senza guadagnare nulla.
            ...
            if not room_ok:
                self.log("W", f"AUTO skip [{self.strategy}] {symbol} {d}: {room_reason}")
                m["last_auto_dir"] = d
                return
