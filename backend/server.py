...
def check_filters(vol_info: dict, min_index: float = 0.6):
    if vol_info.get("level") == "compressa" or vol_info.get("index", 1.0) < min_index:
        return False, f"Volatilità troppo compressa (indice {vol_info.get('index', 0):.2f}) — mercato piatto, rischio di oscillare senza risolversi"
    if not in_session():
...
class DerivClient:
    ...
    def __init__(self):
        ...
        # Uscita intelligente: invece di aspettare sempre il TP/SL pieno, blocca un guadagno
        # più piccolo se il prezzo comincia a tornare indietro dopo essere stato in profitto,
        # e taglia una perdita più piccola se il mercato continua chiaramente contro la
        # posizione, invece di aspettare la perdita massima. Il TP/SL pieno resta comunque
        # come rete di sicurezza finale se nessuna delle due condizioni scatta prima.
        self.trailing_activation_pct: float = 40.0  # profitto (%) a partire dal quale si attiva il trailing
        self.trailing_giveback_pct: float = 20.0    # quanti punti % può ridarsi indietro dal massimo prima di chiudere
        self.early_cut_pct: float = 40.0            # perdita (%) oltre la quale si valuta il taglio anticipato
        self.early_cut_confirm: int = 3             # letture consecutive in perdita oltre la soglia per confermare (evita chiusure per un singolo sbalzo)
        # Indice di volatilità minimo (relativo, self-normalizzato per asset) richiesto per
        # aprire un trade auto — sotto questa soglia il mercato è considerato troppo "piatto"
        # e più incline a oscillare senza risolversi. 1.0 = volatilità nella norma per quel
        # simbolo/timeframe; 0.6 = 40% sotto la norma recente.
        self.min_volatility_index: float = 0.6
    ...
    async def load_persisted_config(self):
        ...
        if cfg and cfg.get("token"):
            ...
            self.trailing_activation_pct = cfg.get("trailing_activation_pct", self.trailing_activation_pct)
            self.trailing_giveback_pct = cfg.get("trailing_giveback_pct", self.trailing_giveback_pct)
            self.early_cut_pct = cfg.get("early_cut_pct", self.early_cut_pct)
            self.early_cut_confirm = cfg.get("early_cut_confirm", self.early_cut_confirm)
            self.min_volatility_index = cfg.get("min_volatility_index", self.min_volatility_index)
            ...
            async for t in db.trades.find({"status": "open"}):
                ...
                if cid:
                    self.pending_trade_meta[cid] = {
                        ...
                        "peak_profit_pct": 0.0, "adverse_streak": 0,
                    }
            ...
            if poc and poc.get("contract_id"):
                ...
                if poc.get("is_sold"):
                    ...
                    if meta and meta.get("source") == "auto" and cid not in self.auto_closing:
                        ...
                        # 1) Trailing: se il profitto ha toccato una soglia significativa e poi
                        # ha ridato indietro troppo dal suo massimo, blocchiamo il guadagno
                        # ridotto invece di rischiare che torni in perdita aspettando il TP pieno.
                        peak = max(meta.get("peak_profit_pct", 0.0), profit_pct)
                        meta["peak_profit_pct"] = peak
                        if peak >= self.trailing_activation_pct and profit_pct <= peak - self.trailing_giveback_pct:
                            close_reason = f"TRAILING (picco {peak:+.1f}%, ora {profit_pct:+.1f}%)"

                        # 2) Taglio anticipato: se il mercato continua chiaramente contro la
                        # posizione (non un singolo sbalzo, ma diverse letture di fila), meglio
                        # una perdita più piccola ora che aspettare lo SL pieno.
                        if close_reason is None:
                            if profit_pct <= -self.early_cut_pct:
                                meta["adverse_streak"] = meta.get("adverse_streak", 0) + 1
                            else:
                                meta["adverse_streak"] = 0
                            if meta["adverse_streak"] >= self.early_cut_confirm:
                                close_reason = f"CUT ANTICIPATO ({profit_pct:+.1f}%, confermato su {meta['adverse_streak']} letture)"

                        # 3) Rete di sicurezza finale: TP/SL pieno, come prima — se per qualsiasi
                        # motivo i due controlli sopra non sono scattati.
                        if close_reason is None and (profit_pct >= self.auto_tp_pct or profit_pct <= -self.auto_sl_pct):
                            close_reason = "TP" if profit_pct >= self.auto_tp_pct else "SL"

                        if close_reason is not None:
                            self.auto_closing.add(cid)
                            self.log("S", f"AUTO {close_reason} #{cid}: {profit_pct:+.1f}% — chiudo")
    ...
    async def _maybe_signal(self, symbol: str):
        ...
        if len(candles) >= 25:
            ...
            # Volatilità: calcolata per OGNI simbolo della watchlist (non solo quello attivo),
            # così il pannello di confronto nel frontend ha dati per tutti gli asset.
            m["volatility"] = strategies.volatility_index(candles)
            ...
            if symbol == self.active_symbol:
                ...
                ok_d, reason_d = check_filters(m["volatility"], self.min_volatility_index)
                self.filter_ok = ok_d
    ...
    async def _maybe_signal_configs(self, symbol: str):
        ...
        for cfg in configs:
            ...
            ind = compute_indicators(candles)
            vol_info = strategies.volatility_index(candles)
            tf["volatility"] = vol_info
            ok, reason = check_filters(vol_info, self.min_volatility_index)
    ...
    def get_state(self):
        ...
        return {
            ...
            "volatility": active_m.get("volatility", {"index": 1.0, "level": "normale"}),
            ...
            "markets": {
                sym: {
                    ...
                    "signals": mk["signals"],
                    "volatility": mk.get("volatility", {"index": 1.0, "level": "normale"}),
                }
            ...
            "early_cut_pct": self.early_cut_pct,
            "early_cut_confirm": self.early_cut_confirm,
            "min_volatility_index": self.min_volatility_index,
...
class AutoSettingsBody(BaseModel):
    ...
    early_cut_pct: Optional[float] = None
    early_cut_confirm: Optional[int] = None
    min_volatility_index: Optional[float] = None
...
async def set_auto_settings(body: AutoSettingsBody):
    ...
    if body.trailing_activation_pct is not None:
        if body.trailing_activation_pct <= 0:
            raise HTTPException(400, "trailing_activation_pct deve essere positivo")
        client.trailing_activation_pct = body.trailing_activation_pct
        updates["trailing_activation_pct"] = body.trailing_activation_pct
    if body.trailing_giveback_pct is not None:
        if body.trailing_giveback_pct <= 0:
            raise HTTPException(400, "trailing_giveback_pct deve essere positivo")
        client.trailing_giveback_pct = body.trailing_giveback_pct
        updates["trailing_giveback_pct"] = body.trailing_giveback_pct
    if body.early_cut_pct is not None:
        if body.early_cut_pct <= 0:
            raise HTTPException(400, "early_cut_pct deve essere positivo")
        client.early_cut_pct = body.early_cut_pct
        updates["early_cut_pct"] = body.early_cut_pct
    if body.early_cut_confirm is not None:
        if body.early_cut_confirm < 1:
            raise HTTPException(400, "early_cut_confirm deve essere almeno 1")
        client.early_cut_confirm = body.early_cut_confirm
        updates["early_cut_confirm"] = body.early_cut_confirm
    if body.min_volatility_index is not None:
        if body.min_volatility_index <= 0:
            raise HTTPException(400, "min_volatility_index deve essere positivo")
        client.min_volatility_index = body.min_volatility_index
        updates["min_volatility_index"] = body.min_volatility_index
