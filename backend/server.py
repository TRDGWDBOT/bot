...
SYMBOLS = {
    ...
    "LTCUSD": "cryLTCUSD",
    "XRPUSD": "cryXRPUSD",
    # NB: indici azionari (US500/US100/US30/GER40/UK100/JPN225) rimossi apposta:
    # seguono orari di borsa limitati ed erano causa di errori "Trading is not
    # offered for this duration" e di posizioni chiuse a mercato fermo.
}
...
class DerivClient:
    ...
    def __init__(self):
        ...
        self.market_open: dict[str, bool] = {}  # simbolo Deriv -> mercato aperto adesso? (indici azionari hanno orari, forex/crypto/oro no)
        self.req_id = 100
        ...
        while True:
            ...
            try:
                ...
                async with websockets.connect(url, ping_interval=None) as ws:
                    ...
                    async def _refresh_market_sessions():
                        # Gli indici azionari (GER40, UK100, US500, ecc.) seguono l'orario della
                        # borsa reale e possono aprire/chiudere più volte durante la giornata.
                        # Forex/crypto/oro/argento sono H24 (quasi). Ricontrolliamo periodicamente
                        # quali simboli sono negoziabili ADESSO, per evitare di generare segnali/
                        # ordini su mercati chiusi (causa dell'errore "Trading is not offered for
                        # this duration" e di posizioni "scadute" a mercato chiuso).
                        while True:
                            try:
                                await self._refresh_active_symbols()
                            except Exception as e:
                                self.log("W", f"Refresh sessioni mercato fallito: {e}")
                    ...
                    sessions_task: Optional[asyncio.Task] = None
                    ...
                    try:
                        ...
                        # e registra quali sono negoziabili ADESSO (exchange_is_open).
                        await self._refresh_active_symbols(initial=True)
                        sessions_task = asyncio.create_task(_refresh_market_sessions())
    ...
    async def _refresh_active_symbols(self, initial: bool = False):
        """Aggiorna self.watchlist (simboli disponibili sul conto) e self.market_open
        (simboli negoziabili ADESSO, in base all'orario della borsa/mercato). Va richiamata
        periodicamente perché gli indici azionari aprono/chiudono durante la giornata,
        mentre forex/crypto/oro restano quasi sempre aperti.
        Tutto il corpo è protetto da un unico try/except: un problema qui non deve mai
        far cadere l'intera connessione WebSocket."""
        try:
            active = await self._send({"active_symbols": "brief"}, timeout=25)
            if active.get("error"):
                raise RuntimeError(active["error"].get("message", "errore active_symbols"))

            by_symbol = {s["symbol"]: s for s in active.get("active_symbols", []) if s.get("symbol")}

            if initial:
                self.watchlist = []
                for name, deriv_sym in SYMBOLS.items():
                    if deriv_sym in by_symbol:
                        self.watchlist.append(deriv_sym)
                    else:
                        self.log("W", f"Simbolo non disponibile sul conto, saltato: {name} ({deriv_sym})")

            new_open: dict[str, bool] = {}
            for name, deriv_sym in SYMBOLS.items():
                info = by_symbol.get(deriv_sym)
                if info is None:
                    continue
                new_open[deriv_sym] = bool(info.get("exchange_is_open", 1))

            # Log solo le transizioni (mercato che apre/chiude), non ripetiamo ogni 5 minuti
            for deriv_sym, is_open in new_open.items():
                was_open = self.market_open.get(deriv_sym, is_open)
                if was_open != is_open:
                    name = SYMBOL_TO_NAME.get(deriv_sym, deriv_sym)
                    self.log("I", f"Mercato {'aperto' if is_open else 'chiuso'}: {name} ({deriv_sym})")

            self.market_open = new_open
        except Exception as e:
            if initial:
                self.log("W", f"active_symbols non risposto ({e}) — uso lista completa")
                self.watchlist = list(SYMBOLS.values())
                self.market_open = {sym: True for sym in SYMBOLS.values()}
            else:
                self.log("W", f"Refresh sessioni mercato fallito: {e}")
    ...
    async def _maybe_signal(self, symbol: str):
        ...
        if self.auto_mode and confirmed and ok and d != "WAIT" and d != m["last_auto_dir"]:
            if not self.market_open.get(symbol, True):
                # Mercato chiuso adesso (es. indice azionario fuori orario di borsa):
                # niente segnale/ordine, evitiamo l'errore "Trading is not offered for this
                # duration" e posizioni che scadono a mercato chiuso.
                m["last_auto_dir"] = d
                return
    ...
    async def place_order(self, direction: str, stake: float = 1.0, multiplier: int = 100, symbol: Optional[str] = None, source: str = "manual", strategy: Optional[str] = None):
        ...
        if not self.market_open.get(sym, True):
            name = SYMBOL_TO_NAME.get(sym, sym)
            raise RuntimeError(f"Mercato chiuso per {name} adesso — ordine non inviato")
