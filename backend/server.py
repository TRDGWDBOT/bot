...
async def _fetch_candles_paginated(sym: str, granularity: int, total_count: int) -> list:
    """Deriv limita ogni singola chiamata ticks_history a ~5000 candele. Per andare oltre,
    concateniamo più chiamate: la prima prende le più recenti, le successive risalgono nel
    tempo usando "end" = timestamp della candela più vecchia già ottenuta meno un intervallo.
    Si ferma se raggiunge il totale richiesto o se Deriv non ha più storico da restituire."""
    all_candles: list = []
    ...
    while len(all_candles) < total_count:
        ...
        if len(batch) < batch_count:
            break  # Deriv non ha più storico oltre questo punto
    return all_candles
