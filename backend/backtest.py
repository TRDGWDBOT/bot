...
def _simulate_one_strategy(candles: List[dict], strategy_name: str, confirm_need: Optional[int],
                            ...
                            min_volatility_index: float = 0.6, warmup: int = 60) -> Dict:
    ...
    # IMPORTANTE per fedeltà al bot live, non solo per velocità: dal vivo il buffer di
    # candele per simbolo è limitato alle ultime 200 (vedi server.py, _update_pseudo_candle).
    # Nessuna strategia guarda comunque più indietro di ~50-60 candele per i suoi indicatori
    # (EMA50, RSI14, Bollinger20, Donchian20...), quindi una finestra di 200 è più che
    # sufficiente E rende ogni valutazione O(1) invece di O(i) — su 10000 candele passare da
    # una finestra crescente (O(n²) totale) a una finestra fissa taglia il tempo di calcolo
    ...
    while i < n:
        ...
        if confirmed and d != "WAIT" and d != last_dir:
            ...
            # Stesso filtro del bot live: salta i trade in mercati troppo compressi/piatti.
            # NB: qui NON replichiamo il filtro "fuori sessione" del live, perché non ha senso
            # applicato a candele storiche già passate — quello resta specifico del trading reale.
...
def run_backtest(candles: List[dict], strategy_names: List[str], confirm_need: Optional[int],
                  tp_pct: float, sl_pct: float, multiplier: int, stake: float,
                  min_volatility_index: float = 0.6) -> Dict:
    results = {name: _simulate_one_strategy(candles, name, confirm_need, tp_pct, sl_pct, multiplier, stake, min_volatility_index)
    ...
    return {
        ...
        "params": {
            ...
            "min_volatility_index": min_volatility_index,
        },
        "results": results,
    }
