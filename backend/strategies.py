...
# Punteggio minimo che un segnale ICT o PA "da solo" deve avere per essere considerato
# abbastanza deciso da usare la combinata anche quando l'altra strategia è semplicemente
# in attesa (non in disaccordo attivo). Richiedere che le due diano segnale ESATTAMENTE
# sulla stessa candela le rende quasi mai simultanee — è un AND troppo rigido tra due
# framework indipendenti che raramente si sincronizzano al singolo tick.
SOLO_STRONG_THRESHOLD = 6
...
def combined_signal(candles: List[dict]) -> Dict:
    ...
    both_wait = ict["dir"] == "WAIT" and pa["dir"] == "WAIT"
    agree = ict["dir"] == pa["dir"] and ict["dir"] != "WAIT"
    conflict = (ict["dir"] != "WAIT" and pa["dir"] != "WAIT" and ict["dir"] != pa["dir"])

    if agree:
        # Le due sono d'accordo sulla stessa candela: il caso "forte", conferma piena.
        direction = ict["dir"]
        score = ict["score"] + pa["score"]
        conf_divisor = 15
    elif conflict:
        # Disaccordo attivo: qui sì che è un campanello d'allarme vero, si aspetta.
        direction, score, conf_divisor = "WAIT", 0, 15
    elif both_wait:
        direction, score, conf_divisor = "WAIT", 0, 15
    else:
        # Una delle due è decisa, l'altra semplicemente non ha nulla da dire (non è in
        # disaccordo). Se il segnale solitario è abbastanza forte, lo usiamo comunque,
        # ma con confidenza ridotta perché non è doppiamente confermato.
        solo = ict if ict["dir"] != "WAIT" else pa
        if abs(solo["score"]) >= SOLO_STRONG_THRESHOLD:
            direction = solo["dir"]
            score = solo["score"]
            conf_divisor = 20  # divisore più alto = confidenza max più bassa a parità di score
        else:
            direction, score, conf_divisor = "WAIT", 0, 15

    conf = min(100, round(abs(score) / conf_divisor * 100)) if direction != "WAIT" else 0
    return {"dir": direction, "score": score, "conf": conf, "reasons": reasons}
