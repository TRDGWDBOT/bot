...
def target_reachable(candles: List[dict], direction: str, tp_distance: float, sl_distance: float,
    ...
    """Controlla se, in base alla struttura di mercato recente (swing high/low) e alla
    volatilità (ATR), il prezzo sembra avere margine per raggiungere il TP nella direzione
    del segnale PRIMA di incontrare un ostacolo che potrebbe farlo tornare indietro fino
    allo SL. Non è una previsione garantita — nessun indicatore lo è — ma un filtro di
    buon senso su due cose osservabili adesso:
      1) c'è un livello di resistenza/supporto noto (swing recente) tra il prezzo attuale
         e il target di TP? Se sì, è un ostacolo plausibile prima di arrivarci.
      2) il TP richiede un movimento molto più ampio della volatilità media recente (ATR)?
         Se sì, è statisticamente meno probabile arrivarci senza prima oscillare fino allo SL.
    Ritorna (ok, motivo_se_bloccato).
    """
    if tp_distance <= 0 or direction not in ("BUY", "SELL") or len(candles) < 20:
        return True, ""
    ...
    if direction == "BUY":
        ...
        obstacles = [s["price"] for s in swings if s["type"] == "high" and price < s["price"] < tp_target]
        if obstacles:
            nearest = min(obstacles)
            return False, f"resistenza a {nearest:.5f} prima del TP ({tp_target:.5f})"
    else:  # SELL
        tp_target = price - tp_distance
        obstacles = [s["price"] for s in swings if s["type"] == "low" and tp_target < s["price"] < price]
        if obstacles:
            nearest = max(obstacles)
            return False, f"supporto a {nearest:.5f} prima del TP ({tp_target:.5f})"

    if atr_val and atr_val > 0 and tp_distance > atr_val * MAX_TP_ATR_MULT:
        return False, f"TP troppo lontano per la volatilità recente (ATR={atr_val:.5f})"
